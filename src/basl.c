    ...
    uint32_t *const off)
{
	...
	*off = table->off + table->len;
	...
	BASL_EXEC(qemu_fwcfg_add_file((const char *)table->fwcfg_name,
	...
	if (gva == NULL) {
		warnx("%s: could not map gpa [ 0x%16llx, 0x%16llx ]", __func__,
...
basl_finish_patch_checksums(struct basl_table *const table)
{
	...
	STAILQ_FOREACH(checksum, &table->checksums, chain) {
		...
		if (gva == NULL) {
			warnx("%s: could not map gpa [ 0x%16llx, 0x%16llx ]",
...
basl_get_table_by_signature(const uint8_t signature[ACPI_NAMESEG_SIZE])
{
	...
	STAILQ_FOREACH(table, &basl_tables, chain) {
		...
		if (strncmp((const char *)header->Signature,
...
{
	BASL_EXEC(basl_table_create(&rsdt, ctx, (const uint8_t *)ACPI_SIG_RSDT,
	    BASL_TABLE_ALIGNMENT));

	/* Header */
	BASL_EXEC(basl_table_append_header(rsdt, (const uint8_t *)ACPI_SIG_RSDT,
	    1, 1));
...
{
	BASL_EXEC(basl_table_create(&xsdt, ctx, (const uint8_t *)ACPI_SIG_XSDT,
	    BASL_TABLE_ALIGNMENT));

	/* Header */
	BASL_EXEC(basl_table_append_header(xsdt, (const uint8_t *)ACPI_SIG_XSDT,
	    1, 1));
	/* Pointers (added by basl_table_register_to_rsdt) */
...
basl_init(struct vmctx *const ctx)
{
	...
	BASL_EXEC(qemu_loader_create(&basl_loader,
	    (const uint8_t *)QEMU_FWCFG_FILE_TABLE_LOADER));
...
basl_table_register_to_rsdt(struct basl_table *table)
{
	...
	BASL_EXEC(basl_table_append_pointer(rsdt,
	    (const uint8_t *)header->Signature, ACPI_RSDT_ENTRY_SIZE));
	BASL_EXEC(basl_table_append_pointer(xsdt,
	    (const uint8_t *)header->Signature, ACPI_XSDT_ENTRY_SIZE));
