...
void
json_enable_debug(int level, FILE *fp)
...
static void
json_trace(int errlevel, const char *fmt, ...)
	...
	for (; *cp != '\0'; cp++) {
		...
		case await_value:
			if (isspace((unsigned char)*cp) || *cp == ':') {
				...
				if (cursor->type != t_array) {
					json_debug_trace((1,
					    "Saw [ when not expecting "
					    "array.\n"));
					if (end != NULL)
						*end = cp;
					return JSON_ERR_NOARRAY;
				}
				substatus = json_read_array(cp,
				    &cursor->addr.array, &cp);
				if (substatus != 0) {
					return substatus;
				}
				state = post_element;
				...
				substatus = json_read_object(cp,
				    cursor->addr.attrs, &cp);
				if (substatus != 0) {
					return substatus;
				}
		...
		case post_val:
			...
			if (value_quoted &&
				...
				json_debug_trace((1,
				    "Saw quoted value when expecting"
				    " non-string.\n"));
			...
			if (cursor->type == t_check) {
				...
				lptr = json_target_address(cursor, parent,
				    offset);
			}
			if (lptr != NULL) {
				switch (cursor->type) {
				...
				case t_uinteger: {
					unsigned int tmp = (unsigned int)atoi(
					    valbuf);
					memcpy(lptr, &tmp,
					    sizeof(unsigned int));
				} break;
				...
				case t_ushort: {
					unsigned short tmp = (unsigned int)atoi(
					    valbuf);
					memcpy(lptr, &tmp,
					    sizeof(unsigned short));
				} break;
    ...
    const char **end)
{
	...
	for (offset = 0; offset < arr->maxlen; offset++) {
		...
		case t_object:
		case t_structobject:
			substatus = json_internal_read_object(cp,
			...
			break;
		case t_integer:
			arr->arr.integers.store[offset] = (int)strtol(cp, &ep,
			    0);
			if (ep == cp) {
			...
			break;
		case t_uinteger:
			arr->arr.uintegers.store[offset] = (unsigned int)
			    strtoul(cp, &ep, 0);
			if (ep == cp) {
				return JSON_ERR_BADNUM;
			} else {
			...
			break;
		case t_short:
			arr->arr.shorts.store[offset] = (short)strtol(cp, &ep,
			    0);
			if (ep == cp) {
				return JSON_ERR_BADNUM;
			} else {
			...
			break;
		case t_ushort:
			arr->arr.ushorts.store[offset] = (unsigned short)
			    strtol(cp, &ep, 0);
			if (ep == cp) {
				return JSON_ERR_BADNUM;
			} else {
		...
		case t_time:
			...
			arr->arr.reals.store[offset] = iso8601_to_unix(
			    (char *)cp);
		...
		case t_boolean:
			if (str_starts_with(cp, "true")) {
				...
				if (ep == cp) {
					...
					arr->arr.booleans.store[offset] = (bool)
					    val;
					cp = ep;
				}
			}
