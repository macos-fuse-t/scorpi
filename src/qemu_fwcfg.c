...
#else
static int
fwcfg_selector_handler(int dir, uint64_t *val, int bytes)
{
	if (bytes != sizeof(uint16_t)) {
		warnx("%s: invalid size (%d) of fwcfg access", __func__, bytes);
		return (-1);
	}

	if (dir == MEM_F_WRITE) {
		fwcfg_sc.data_offset = 0;
		fwcfg_sc.selector.bits = SwapBytes16(*val);
	} else {
		*val = SwapBytes16(fwcfg_sc.selector.bits);
	}
	return (0);
}

static int
fwcfg_data_handler(int dir, uint64_t *val, int bytes)
{
	if (dir == MEM_F_WRITE) {
		warnx("%s: Writes to qemu fwcfg data port aren't allowed",
		    __func__);
	...
	/* return item data */
	memcpy(val, &item->data[fwcfg_sc.data_offset], bytes);
	fwcfg_sc.data_offset += bytes;

	return (0);
}

static int
mmio_fwcfg_mem_handler(struct vcpu *vcpu __unused, int dir, uint64_t addr,
    int size, uint64_t *val, void *arg1 __unused, long arg2 __unused)
{
	uint64_t offset = addr - fwcfg_sc.mmio_base;
	if (!offset) {
		return fwcfg_data_handler(dir, val, size);
	}

	return fwcfg_selector_handler(dir, val, size);
}
...
qemu_fwcfg_add_file(const char *name, const uint32_t size, void *const data)
{
	...
	for (file_index = 0; file_index < count - 1; ++file_index) {
		if (strcmp(name,
			(const char *)fwcfg_sc.directory->files[file_index]
			    .name) < 0)
			break;
	}
	...
	strcpy((char *)fwcfg_sc.directory->files[file_index].name, name);
...
qemu_fwcfg_add_user_files(void)
{
	...
	STAILQ_FOREACH(fwcfg_file, &user_files, chain) {
		error = qemu_fwcfg_add_file((const char *)fwcfg_file->name,
...
int
qemu_fwcfg_init(struct vmctx *const ctx, uint64_t mmio_base, size_t mmio_size)
{
	int error;
	bool fwcfg_enabled;
	struct mem_range mr;
...
#else
		if (mmio_base) {
			fwcfg_sc.mmio_base = mmio_base;
			bzero(&mr, sizeof(struct mem_range));
			mr.name = "uart";
			mr.base = mmio_base;
			mr.size = mmio_size;
			mr.flags = MEM_F_RW;
			mr.handler = mmio_fwcfg_mem_handler;
			mr.arg1 = ctx;
			mr.arg2 = mr.base;
			error = register_mem(&mr);
			if (error) {
				warnx(
				    "%s: failed to register memory for QEMU FwCfg",
				    __func__);
				goto done;
			}
		}
